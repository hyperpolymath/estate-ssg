# MCP Adapter for estate-ssg

This directory contains the **ReScript MCP adapter** that connects estate-ssg to the poly-ssg-mcp hub.
...
adapters/
├── src/
│   └── EstateAdapter.res  # ReScript adapter source
...
**estate-ssg uses Forth** as its primary implementation language.
The adapter wraps the Forth runtime to provide MCP protocol access.

## Connection to Hub

This adapter exports:
- `name` - Satellite identifier
- `language` - Primary implementation language (Forth)
...
ALL SSG logic must remain in Forth.
The adapter may ONLY:
- Execute commands in the Forth runtime
